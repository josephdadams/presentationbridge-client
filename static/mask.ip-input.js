;(function ($) {
	...
	String.prototype.getSymbolCount = function (char = '.') {
		...
		for (let index = charArray.indexOf(char); index < charArray.length; index++) {
	...
	$.fn.ipAddress = function () {
		...
		this.each(function (index, item) {
			if ($(item).attr('placeholder') == undefined) $(item).attr('placeholder', '000.000.000.000')
		})
		...
		$(this).on('keydown', function (e) {
			...
			if (that.value.length >= 15 || (that.newValue.getSymbolCount('.') >= 3 && that.value.length - pointIndex >= 3))
				return false

			if (
				!/[0-9\.]/.test(e.key) ||
				((that.newValue[that.newValue.length - 1] == '.' || that.value == '' || that.value.getSymbolCount('.') >= 3) &&
					e.key == '.')
			) {
				$(this).addClass('is-invalid')
				return false
			...
			if (
				(that.value + (e.key == '.' || that.value.length - pointIndex == 3 ? '.' : e.key))
					.split('.')
