				...
				.then((v) => {
					if (v.response == 0) {
						dialog.showMessageBox(settingsWindow, {
							...
							message:
								'The update is being downloaded in the background. Once finished, you will be prompted to save your work and restart PresentationBridge Client.',
						})
...
function presentationbridge_gotologo() {
	...
	trayMenuItems[3].click = function () {
...
function turnOffCommands() {
	...
	trayMenuItems[4].click = function () {
		turnOnCommands()
	}
	buildTray()
}
...
function turnOffLyrics() {
	...
	trayMenuItems[5].click = function () {
		turnOnLyrics()
	}
	buildTray()
}
	...
	axios(options)
		.then(function (response) {
			if (response.data !== undefined && response.data !== null) {
				bridgeIO.emit(
					'current_slide_image',
					config.get('presentationbridgeID'),
